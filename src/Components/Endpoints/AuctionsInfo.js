...
const Hero = () => {
  ...
  const handleClick = (params) => {
    POST({ payload: params, endpoint:"auctions/info", setResponse, setError});
  }
  ...
  const getBidsInfo = () => {
    ...
    if (bidId === "") {
      alert("Please enter a auction ID")
      setIsLoading(false);
      return
    }
    PostAuctionsInfo({address, bidId, setPayload, setMessage});
  }
    ...
    <VStack spacing={4} p={8} minWidth={"600px"} maxWidth={"700px"}  alignItems={"left"} >
        ...
        <Box width={"100%"} bgColor={COLORS.main} borderRadius={"10"}>
          <VStack spacing={3} direction="row" align="left" m="1rem">
              <InfoRow bgColor="green.500" header={"HTTP Request"} text={"POST https://oev.api3dev.com/api/auctions/info"}></InfoRow>
              <BidInfoParams id={bidId} setBidId={setBidId} paramName={"Auction Id"}></BidInfoParams>
